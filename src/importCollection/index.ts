...
function writeCollection(data: JSON, path: string): Promise<any> {
  return new Promise(async (resolve, reject) => {
    ...
    for (let [id, item] of Object.entries(data)) {
      ...
      const subColKeys = Object.keys(item).filter((k) =>
        k.startsWith(args.collPrefix + ':')
      );
...
function datafromCSV(file: string) {
  ...
  console.log(json);
  // TODO parse "TRUE" to true and "FALSE" to false
  return dataFromJSON(json);
}
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (mode === 'array') {
      ...
      if (coll === '/' || collections.length > 1 || isDocumentPath(coll)) {
        reject(
          'Specify single target collection path for import of JSON array of documents.'
        );
        return;
      }
      data[coll] = json;
    ...
    const rootJsonCollections = Object.keys(json).filter((k) =>
      k.startsWith(args.collPrefix + ':')
    );
    ...
    if (rootJsonCollections.length === 0) {
      ...
      if (coll === '/' || collections.length > 1 || isDocumentPath(coll)) {
        reject(
          'Specify single target collection path for import of JSON keyed object documents.'
        );
        return;
      }
    ...
    if (collections[0] !== '/') {
      collections.forEach((collection) => {
        ...
        if (!coll) {
          reject(
            `Source JSON file contains no collection named: ${collection}`
          );
          return;
        }
  ...
  return new Promise((resolve, reject) => {
    ...
    if (!file.endsWith('INDEX.csv')) {
      ...
      if (collections.length > 1) {
        reject(
          'Multiple collection import from CSV requires an *.INDEX.csv file.'
        );
        return;
      }
    ...
    if (collections[0] !== '/') {
      args.verbose &&
        console.log(`Mode: Selected collections from Indexed CSV`);
      collections.forEach((collection) => {
        const colls = index.filter((coll) =>
          (coll['Collection'] + '/').startsWith(collection + '/')
        );
        if (colls.length) {
          colls.forEach((coll) => {
...
function readXLSXBook(path, collections: string[]): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    if (collections[0] !== '/') {
      args.verbose &&
        console.log('Mode: Selected Sheets from indexed XLSX Workbook');
      collections.forEach((collection) => {
        const colls = index.filter((coll) =>
          (coll['Collection'] + '/').startsWith(collection + '/')
        );
    ...
    if (collections[0] === '/') {
      args.verbose &&
        console.log('Mode: All Sheets from indexed XLSX Workbook');
      const collection = collections[0];
      _.forEach(index, (coll) => {
        const sheetName = coll['Sheet Name'];
